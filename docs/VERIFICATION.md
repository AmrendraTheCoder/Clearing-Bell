...
- Added a lazy, cancellable scene adapter,25-second loading timeout/retry, play/pause, reset, still preview, mobile/reduced-motion opt-in, and offscreen/hidden suspension. Independent source review identified and resolved stale StrictMode callbacks, queued-start timing and late disposal issues. Paused resizing defers drawing-buffer changes until resume.
